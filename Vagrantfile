...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define MACHINE[i] do |server|
    ...
    # server.vm.network "private_network", type: "dhcp", auto_config: true
    server.vm.network "public_network", auto_config: false
    # , ip: "192.168.10.#{IP[i]}", gateway: "192.168.10.254"
    # , netmask: "255.255.255.0"

    # server.ssh.host = "192.168.10.#{IP[i]}"

    server.vm.synced_folder "/Users/gnammyx/Parallels/volumes/#{MACHINE[i]}", "/srv", type: "nfs", nfs_udp: false, map_uid: 0, map_gid: 0
    ...
    server.vm.provider "parallels" do |prl|
      ...
      prl.customize "post-comm", ["set", :id,
                                  "--device-set", "net1",
                                  "--type", "bridged",
                                  "--iface", "default",
    ...
    #   host_shell.inline = "prlctl set #{MACHINE[i]} --device-set net0 --type shared --dhcp yes"
    # end
    # server.vm.provision :host_shell do |host_shell|
    #   host_shell.inline = "prlctl set #{MACHINE[i]} --device-set net1 --type bridged --iface default --ipadd 192.168.10.#{IP[i]} --dhcp no --gw 192.168.10.254 --nameserver 192.168.1.1"
    # end
  end
  end
  ...
  #   run: "always",
  #   inline: "route add default gw 192.168.10.254"
